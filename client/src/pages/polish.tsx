...
import { PolishResult } from "@shared/schema";
...
export default function PolishPage() {
  ...
  const { data: results, isLoading, error } = useQuery<PolishResult[]>({
    queryKey: ["/api/polish", blockNo, partNo, thickness],
    enabled: blockNo.trim().length > 0,
    queryFn: async () => {
      return getPolishData(
        blockNo.trim(),
  ...
  // No header colors needed for polish page
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b sticky top-0 bg-background z-10">
        <div className="container mx-auto px-0 h-12 flex items-center justify-between">
          <h1 className="text-xl font-bold pl-2">Polish Search</h1>
          <div className="flex items-center gap-2">
            ...
            >
              {logoutMutation.isPending ? (
                <Loader2 className="h-4 w-4 animate-spin text-[#02066F]/50" />
              ) : (
          ...
          <CardContent className="p-1">
                ...
                <div className="space-y-1">
                  <div className="relative">
                    <Input
                      ...
                      className="h-9 text-sm pr-8 border-[#02066F]/20 focus:border-[#02066F]/40 hover:border-[#02066F]/30"
                    />
                    <Button
                      variant="ghost"
                      size="icon"
                      className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7 hover:bg-[#02066F]/5"
                      onClick={() => startListening('blockNo')}
                      disabled={isListening}
                    >
                      {isListening && activeInput === 'blockNo' ? (
                        <MicOff className="h-4 w-4 text-[#02066F]/80" />
                      ) : (
                        <Mic className="h-4 w-4 text-[#02066F]/60 hover:text-[#02066F]/80" />
                      )}
                    </Button>
                  </div>
            ...
            <div className="flex justify-end gap-2 mt-2 mb-4">
              <Button 
                ...
                size="sm" 
                onClick={() => setLocation('/')} 
                className="h-9 flex gap-2 items-center bg-[#02066F]/10 hover:bg-[#02066F]/20 text-[#02066F] hover:text-[#02066F] border-[#02066F]/20 hover:border-[#02066F]/30"
              >
              ...
              <Button 
                ...
                size="sm" 
                onClick={handleClear} 
                className="h-9 bg-[#02066F] hover:bg-[#02066F]/90 text-white"
              >
                Clear
              </Button>
            </div>
            ...
            {isLoading ? (
              <div className="flex justify-center my-8">
                <Loader2 className="h-8 w-8 animate-spin text-[#02066F]" />
              </div>
            ) : results?.length ? (
              <Table>
                <TableHeader>
                  <TableRow className="bg-[#02066F] hover:bg-[#02066F]/90">
                ...
                <TableBody>
                  {results.map((result, i) => (
                    <TableRow key={i} className={`${i % 2 === 0 ? "bg-white hover:bg-[#02066F]/5" : "bg-[#02066F]/5 hover:bg-[#02066F]/10"}`}>
                      {visibleColumns.map(column => (
